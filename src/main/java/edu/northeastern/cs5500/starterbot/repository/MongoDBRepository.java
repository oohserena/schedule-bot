...
public class MongoDBRepository<T extends Model> implements GenericRepository<T> {
    ...
    public MongoDBRepository(Class<T> clazz, MongoDBService mongoDBService) {
        ...
        String className = clazz.getName().substring(lastDotIndex + 1);
        collection = mongoDatabase.getCollection(className, clazz);
    }
    ...
    public MongoCollection<Interviewer> getCollection() {
        return (MongoCollection<Interviewer>) this.collection;
    }
}
